...
const SentTicket = () => {
    ...
    const { call: get_sent_ticket_list } = useFrappePostCall("internal_ticketing.ticketing_api.get_ticket_created_by_user");

    useEffect(() => {
        get_sent_ticket_list({
            user_id: Cookies.get('user_id')
        }).then((data) => {
            setSentTicketList(data);
        });
    }, []);
        ...
        <div className="w-full">
            ...
            <div className="p-4 shadow-2xl rounded-lg bg-white mt-3">
                <div className="flex justify-between items-center px-4 my-2">
                    <h2 className="text-xl font-bold" >{sentTicketList.message ? "Sent Tickets" : "Sent Tickets"}</h2>
                </div>
                ...
                <div className="p-2 rounded-lg bg-white cursor-pointer overflow-y-auto max-h-[600px] scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 flex flex-col gap-1 mt-2">
                    {sentTicketList.message ?
                        (

                            (sentTicketList.message.map((ticket) => (
